...
public class PlanificarProsupustoRenglonVarianteController implements Initializable {
    ...
    public void handleZonasEmpresa(ActionEvent event) {
        ...
        comboZonas.getItems().setAll(reportProjectStructureSingelton.getZonasList(id).parallelStream().filter(st -> !st.equals("Todas")).collect(Collectors.toList()));
    ...
    public void handleNewPlan(ActionEvent event) {
        ...
        try {
            ...
            controller.cargarUnidadeObra(tableUnidades.getSelectionModel().getSelectedItem().getId(), parEm, parZon, parObj, parNi, parEsp, parSub);
