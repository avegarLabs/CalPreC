...
public class SearchSuminitrosToChangeUOController implements Initializable {

    private static SessionFactory sf;
    CambioSuministroCertificacionUOController suministrostoChange;
    @FXML
    private VBox box;
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        dataTable.setOnMouseClicked(event -> {
                    if (event.getClickCount() == 2) {
                        ...
                        suministrostoChange.getSuminitroResult(suministrosView);
    ...
    public void pasController(CambioSuministroCertificacionUOController suministrosInRVController) {

        loadDatos();
        suministrosViewObservableList = FXCollections.observableArrayList();
        suministrosViewObservableList = utilCalcSingelton.getSuministrosViewObservableList(suministrosInRVController.obraToChenge.getSalarioBySalarioId().getTag());
        dataTable.getItems().setAll(suministrosViewObservableList);

        suministrostoChange = suministrosInRVController;
