...
public class CertificarUORenglonesThread extends Thread {
    ...
    public CertificarUORenglonesThread(int unidadId, Integer idcertP, Date desdeDateP, Date hastaDataP, Double cant, Double valTo) {
        idUnid = unidadId;
        idCer = idcertP;
        ini = desdeDateP;
        fin = hastaDataP;
    ...
    public ArrayList<Certificacionrecuo> certificaRenglonVariante(int unidadId, Integer idcertP, Date desdeDateP, Date hastaDataP, Double cant, Double valToPlan) {
        ...
        for (Unidadobrarenglon item : unidadobrarenglonArrayList) {
            ...
            if (item.getConMat().trim().equals("0")) {
            ...
            } else if (item.getConMat().trim().equals("1")) {
                ...
                costCUCSalario = valCucSal * cantidadCert / dispRecBajo;
                planrecuo.setCosto(0.0);
                planrecuo.setCmateriales(costMaterial);
                planrecuo.setCmano(costMano);
                planrecuo.setCequipo(costEquipo);
                planrecuo.setSalario(costSalario);
                planrecuo.setSalariocuc(costCUCSalario);
    ...
    public Double[] getCostosCertificadosRV(int idUnidadObra, int idReng) {
        ...
        try {
            ...
            valCostCertBajo = 0.0;
            List<Object[]> valoresCerts = session.createQuery("SELECT SUM(cmateriales), SUM(cmano), SUM(cequipo), SUM(salario), SUM(salariocuc) FROM Certificacionrecuo WHERE unidadobraId =: id AND renglonId =: idR").setParameter("id", idUnidadObra).setParameter("idR", idReng).getResultList();
            for (Object[] row : valoresCerts) {
