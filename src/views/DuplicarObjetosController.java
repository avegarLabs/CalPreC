...
public class DuplicarObjetosController implements Initializable {
    ...
    public void pasarDatos(Obra obra, Empresaconstructora empresaconstructora, Zonas zonas, Objetos objetos) {
        obraImp = obra;
        ...
        objImp = objetos;
        objField.setText(objetos.getCodigo() + " - " + objetos.getDescripcion());
    ...
    public void handleCargarZonas(ActionEvent event) {
        ...
        empImpModel = empImpModelList.parallelStream().filter(empImpModel1 -> empImpModel1.getCode().trim().equals(comboEmpresa.getValue().trim())).findFirst().orElse(null);
    }
    ...
    public void handleCargarNivel(ActionEvent event) {
        ...
        nivelViewObservableList = modelsImportViewSingelton.getNivelViewObservableList(objImp.getId());
    ...
    private ObjetoImpModel getObj(String code) {
        return modelsImportViewSingelton.getObjetosList(zonaImpModel.getId()).parallelStream().filter(zonas -> zonas.getCode().substring(0, 2).equals(code)).findFirst().orElse(new ObjetoImpModel(0, "NULL"));
    }
    ...
    public void handleImportarComponentes(ActionEvent event) {
        ...
        zonaImpModel = zonaImpModelList.parallelStream().filter(zonaImpModel1 -> zonaImpModel1.getCode().trim().equals(comboZonas.getValue())).findFirst().orElse(null);
        if (obraImp.getTipo().equals("UO")) {
            String[] partObjetos = objField.getText().split(" - ");
            zonaImpModel = zonaImpModelList.parallelStream().filter(zonaImpModel1 -> zonaImpModel1.getCode().trim().equals(comboZonas.getValue())).findFirst().orElse(null);
            // System.out.println(obraImp.getId() + " " + impModel.getId() + " " +  zonaImp.getId() + " "+ objImp.getId());

            if (getObj(partObjetos[0].trim()).getCode().equals("NULL")) {
                Objetos objetos = new Objetos();
                objetos.setCodigo(partObjetos[0]);
                objetos.setDescripcion(partObjetos[1]);
                objetos.setZonasId(zonaImpModel.getId());
                ...
                if (checkuo.isSelected()) {
                    ...
                    unidadobraList = modelsImportViewSingelton.getUnidadobraByObjetosList(obraImp.getId(), empresaImp.getId(), zonaImp.getId(), objImp.getId());
                    unidadobraList.size();
                    try {
                        crateUnidadObraAndEstructura(unidadobraList, 1);
                        Alert alert = new Alert(Alert.AlertType.INFORMATION);
                        alert.setHeaderText("OK!!");
                        alert.setContentText("El objeto: " + objField.getText() + " ha sido duplicado ");
                        alert.showAndWait();

                    } catch (Exception e) {
                        Alert alert = new Alert(Alert.AlertType.ERROR);
                        alert.setHeaderText("Error!!");
                        alert.setContentText("Error al importar el objeto: " + objField.getText());
                        alert.showAndWait();
                    }
                }
            } else if (modelsImportViewSingelton.getUnidadobraByObjetosList(obraImp.getId(), impModel.getId(), zonaImp.getId(), objImp.getId()).size() == 0) {
                if (checkuo.isSelected()) {
                    unidadobraList = new ArrayList<>();
                    unidadobraList = modelsImportViewSingelton.getUnidadobraByObjetosList(obraImp.getId(), empresaImp.getId(), zonaImp.getId(), objImp.getId());
                    unidadobraList.size();
                    idObjImport = objImp.getId();
                    empImpModel = impModel;
                    try {

                        crateUnidadObraAndEstructura(unidadobraList, 2);
                        Alert alert = new Alert(Alert.AlertType.INFORMATION);
                        alert.setHeaderText("OK!!");
                        alert.setContentText("El objeto: " + objField.getText() + " ha sido duplicado ");
                        alert.showAndWait();
                ...
                alert.setContentText(" La zona " + objField.getText() + " esta declarada en la obra " + obraImp.getDescripion());
                alert.showAndWait();
            }
        } else if (obraImp.getTipo().equals("RV")) {
            String[] partObjetos = objField.getText().split(" - ");
            ...
            if (getObj(partObjetos[0].trim()).getCode().equals("NULL")) {
                ...
                nivelespecificoList = modelsImportViewSingelton.getNivelespecificoByObjectsList(obraImp.getId(), empresaImp.getId(), zonaImp.getId(), objImp.getId());
    ...
    private int getIdNivByCode(String code, int iOb) {
        return modelsImportViewSingelton.getNivelbyOb(iOb).parallelStream().filter(niv -> niv.getCodigo().trim().equals(code.trim())).findFirst().map(Nivel::getId).orElse(0);
    ...
    private void crateUnidadObraAndEstructura(List<Unidadobra> unidadobraList, int val) {

        for (Unidadobra unidadobra : unidadobraList) {
            buildUnidadObraEstructure(unidadobra, val);
        }

    }

    private void buildUnidadObraEstructure(Unidadobra unidadobra, int val) {
        ...
        if (val == 1) {
            ...
            int idNiv = getIdNivByCode(unidadobra.getNivelByNivelId().getCodigo().trim(), idObjImport);
            newUnidad.setNivelId(idNiv);
        }
