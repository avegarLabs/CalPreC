...
public class ImportarNivelesController implements Initializable {
    ...
    public void handleCargarNiveles(ActionEvent event) {
        nivelImpModelList = new ArrayList<>();
        obj = objetoImpModelList.parallelStream().filter(objetoImpModel -> objetoImpModel.getCode().equals(comboObjetos.getValue())).findFirst().orElse(null);
        nivelImpModelList = modelsImportViewSingelton.getNivelList(obj.getId());

        niv = nivelImpModelList.get(1);
        old = 1;
        String[] partZonas = niv.getCode().split(" - ");
    ...
    private int getIndex(String code, int val) {
        int index = 0;
        for (int i = 0; i < nivelImpModelList.size() - 1; i++) {
            if (nivelImpModelList.get(i).getCode().substring(0, 2).equals(zonaCode.getText()) && val != nivelImpModelList.size()) {
                index = i;
            }
        }
        return index;
    ...
    public void handleNextZonas(ActionEvent event) {
        ...
        niv = null;
        niv = nivelImpModelList.get(ind + 1);
        String[] partZonas = niv.getCode().split(" - ");
        zonaCode.setText(partZonas[0]);
        zonaDesc.setText(partZonas[1]);
        handleCargarNivel(event);
    ...
    public void handleCargarNivel(ActionEvent event) {
        empImpModel = empImpModelList.parallelStream().filter(empImpModel1 -> empImpModel1.getCode().equals(comboEmpresa.getValue().trim())).findFirst().orElse(null);
        ...
        nivelViewObservableList = modelsImportViewSingelton.getEspecialidadView(obraModel.getId(), empImpModel.getId(), zonaImpModel.getId(), obj.getId(), niv.getId(), obraImp.getTipo().trim());
    ...
    public void handleImportarComponentes(ActionEvent event) {
        if (obraImp.getTipo().equals("UO")) {
            if (getNivel(zonaCode.getText().trim()).getCode().equals("NULL")) {
                Nivel nivel = new Nivel();
                nivel.setCodigo(zonaCode.getText());
                nivel.setDescripcion(zonaDesc.getText());
                ...
                if (checkuo.isSelected()) {
                    ...
                    unidadobraList = modelsImportViewSingelton.getUnidadobraByNivelesList(obraModel.getId(), impModel.getId(), zonaImpModel.getId(), obj.getId(), niv.getId());
                    unidadobraList.size();
                    try {
                        crateUnidadObraAndEstructura(unidadobraList);
                        Alert alert = new Alert(Alert.AlertType.INFORMATION);
                        alert.setHeaderText("OK!!");
                        alert.setContentText("El Nivel: " + zonaCode.getText() + " ha sido importado ");
                        alert.showAndWait();

                    } catch (Exception e) {
                        Alert alert = new Alert(Alert.AlertType.ERROR);
                        alert.setHeaderText("Error!!");
                        alert.setContentText("Error al importar el nivel: " + zonaCode.getText());
                        alert.showAndWait();
                    }
                }
                ...
                alert.setContentText(" El nivel " + zonaCode.getText() + " esta declarado en la obra " + obraImp.getDescripion());
                alert.showAndWait();
            }
        } else if (obraImp.getTipo().equals("RV")) {
            if (getNivel(zonaCode.getText().trim()).getCode().equals("NULL")) {
                ...
                if (checkuo.isSelected()) {
                    ...
                    nivelespecificoList = modelsImportViewSingelton.getNivelEspecificoByNivelesList(obraModel.getId(), impModel.getId(), zonaImpModel.getId(), obj.getId(), niv.getId());
