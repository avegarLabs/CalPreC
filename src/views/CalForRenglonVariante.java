...
/**
 * Esta clase calcula los conceptos del presupusto a nivel de Renglon Variante
 */
public class CalForRenglonVariante extends Thread {
    ...
    public CalForRenglonVariante(RenglonVarianteObraController renglonController, Empresaconstructora empresa, Obra ob) {
        renglonVarianteObraController = renglonController;
    ...
    public Double[] getListValoresGastos(Integer idob, Integer idemp) {
        ...
        try {
            ...
            List<Object[]> rvuo = session.createSQLQuery(" SELECT * FROM get_values_empresa_obra_rv(:idOb, :idEmp)").setParameter("idOb", idob).setParameter("idEmp", idemp).getResultList();
            for (Object[] row : rvuo) {
    ...
    @Override
    public void run() {
        calcValoresdeGastos(renglonVarianteObraController, empresaconstructoraModel, obra);
    }

    public void calcValoresdeGastos(RenglonVarianteObraController controller, Empresaconstructora empresa, Obra obraSend) {
    ...
    private double calcConcept10(double concepto9, Integer tipo) {
        ...
        if (tipo == 1) {
            ...
            double val = 0.06 * concepto9;
            tempValue = Math.round(val * 100d) / 100d;
        } else if (tipo == 17) {
            double val = 0.01 * concepto9;
            tempValue = Math.round(val * 100d) / 100d;
        }
