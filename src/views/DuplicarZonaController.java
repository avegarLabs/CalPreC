...
public class DuplicarZonaController implements Initializable {
    ...
    public void pasarDatos(Obra obra, Empresaconstructora empresaconstructora, Zonas zonas) {
        ...
        zonaFied.setText(zonas.getCodigo() + " " + zonas.getDesripcion());
        zonasDup = zonas;
    ...
    public void handleCargarObjetos(ActionEvent event) {
        ...
        objetosViewObservableList = modelsImportViewSingelton.getObjetosViewObservableList(zonasDup.getId());
    ...
    private ZonaImpModel getZona(String code) {
        return modelsImportViewSingelton.getZonasList(obraModel.getId()).parallelStream().filter(zonas -> zonas.getCode().substring(0, 3).equals(code)).findFirst().orElse(new ZonaImpModel(0, "NULL"));
    }
    ...
    public void handleImportarComponentes(ActionEvent event) {
        ...
        if (obraImp.getTipo().equals("UO")) {
            String[] partZonas = zonaFied.getText().split(" ");
            impModel = empImpModelList.parallelStream().filter(empImpModel -> empImpModel.getCode().equals(comboEmpresa.getValue().trim())).findFirst().orElse(null);
            if (getZona(partZonas[0].trim()).getCode().equals("NULL")) {
                Zonas zonas = new Zonas();
                zonas.setCodigo(partZonas[0]);
                zonas.setDesripcion(partZonas[1]);
                zonas.setObraId(obraModel.getId());
                ...
                if (checkuo.isSelected()) {
                    ...
                    unidadobraList = modelsImportViewSingelton.getUnidadobraList(obraImp.getId(), empresaImp.getId(), zonasDup.getId());
                    unidadobraList.size();
                    try {
                        ...
                        alert.setContentText("La zona: " + zonaFied.getText() + " ha sido duplicada ");
                        alert.showAndWait();

                    } catch (Exception e) {
                        Alert alert = new Alert(Alert.AlertType.ERROR);
                        alert.setHeaderText("Error!!");
                        alert.setContentText("Error al importar la zona: " + zonaFied.getText());
                        alert.showAndWait();
                    }
                }
            } else if (modelsImportViewSingelton.getUnidadobraList(obraImp.getId(), impModel.getId(), zonasDup.getId()).size() == 0) {
                if (checkuo.isSelected()) {
                    unidadobraList = new ArrayList<>();
                    unidadobraList = modelsImportViewSingelton.getUnidadobraList(obraImp.getId(), empresaImp.getId(), zonasDup.getId());
                    unidadobraList.size();
                    idZonaImp = zonasDup.getId();
                    try {
                ...
                alert.setContentText(" La zona " + zonaFied.getText() + " esta declarada en la obra " + obraImp.getDescripion());
                alert.showAndWait();
            }
            ...
            if (getZona(partZonas[0].trim()).getCode().equals("NULL")) {
                ...
                if (checkuo.isSelected()) {
                    ...
                    nivelespecificoList = modelsImportViewSingelton.getNivelespecificoList(obraImp.getId(), empresaImp.getId(), zonasDup.getId());
    ...
    private int getObjectoByCode(String code) {
        return modelsImportViewSingelton.getObjetosbyZona(zonasDup.getId()).parallelStream().filter(obj -> obj.getCodigo().trim().equals(code.trim())).findFirst().map(Objetos::getId).orElse(0);
    }
    ...
    private void buildNIvelEspecificoEstructure(Nivelespecifico nivelespecifico) {
        ...
        nivel.setEmpresaconstructoraId(impModel.getId());
    ...
    private void buildUnidadObraEstructure(Unidadobra unidadobra, int type) {

        Unidadobra newUnidad = new Unidadobra();
        newUnidad.setObraId(obraModel.getId());
        newUnidad.setEmpresaconstructoraId(impModel.getId());
        newUnidad.setZonasId(idZonaImp);
        if (type == 1) {
            int idObj = getObjecto(unidadobra.getObjetosByObjetosId().getCodigo().trim());
            ...
            int idObj = getObjectoByCode(unidadobra.getObjetosByObjetosId().getCodigo().trim());
            newUnidad.setObjetosId(idObj);
            int idNiv = getIdNivByCode(unidadobra.getNivelByNivelId().getCodigo().trim(), idObj);
            newUnidad.setNivelId(idNiv);
        }
