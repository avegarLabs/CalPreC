...
public class definirCodeNivelController implements Initializable {

    ImportarNIVInCalPreCController importar;
    ...
    public void cargarDatos(ImportarNIVInCalPreCController controller, UniddaObraToIMportView uo) {
        importar = controller;
        labelText.setText(" Modificar código: " + uo.getCodigo().getText());
        labelUser.setText(uo.getCodigo().getText());
    ...
    public void handleAddNewCode(ActionEvent event) {
        if (labelUser.getText().length() == 2) {
            ...
            alert.setContentText("El código debe contener solo 2 dígitos");
            alert.showAndWait();
        }
    }
}
