...
public class DuplicarEspecialidadController implements Initializable {
    ...
    public void pasarDatos(Obra obra, Empresaconstructora empresaconstructora, Zonas zonas, Objetos objetos, Nivel nivel, Especialidades especialidades) {
        obraImp = obra;
        empresaImp = empresaconstructora;
        zonaImp = zonas;
        objetoImp = objetos;
        nivetImp = nivel;
        especialImp = especialidades;
        ...
        espCode.setText(especialImp.getCodigo() + " " + especialImp.getDescripcion());
    ...
    public void handleCargarNivel(ActionEvent event) {
        ...
        objetoImpModel = objetoImpModelList.parallelStream().filter(objetoImp -> objetoImp.getCode().equals(comboObjetos.getValue())).findFirst().orElse(null);
    ...
    public void cargarUO(ActionEvent event) {
        if (obraImp.getTipo().equals("UO")) {
            ...
            unidadobraList = modelsImportViewSingelton.getUnidadobraByNivelesList(obraImp.getId(), empresaImp.getId(), zonaImp.getId(), objetoImp.getId(), nivetImp.getId());
        } else if (obraImp.getTipo().equals("RV")) {
            nivelespecificoList = new ArrayList<>();
            nivelespecificoList = modelsImportViewSingelton.getNivelEspecificoByNivelesList(obraImp.getId(), empresaImp.getId(), zonaImp.getId(), objetoImp.getId(), nivetImp.getId());
    ...
    public void handleCargarSub(ActionEvent event) {
        ...
        System.out.println(nivelViewObservableList.size());
    ...
    public void handleImportarComponentes(ActionEvent event) {
        idNivel = nivelImpModelList.parallelStream().filter(nivelImpModel -> nivelImpModel.getCode().equals(comboNivel.getValue())).map(NivelImpModel::getId).findFirst().orElse(0);
        empImpModel = empImpModelList.parallelStream().filter(emp -> emp.getCode().equals(comboEmpresa.getValue().trim())).findFirst().orElse(null);
        ...
        if (obraImp.getTipo().equals("UO")) {
            ...
            alert.setContentText(" Duplicación Treminada ");
            alert.showAndWait();
