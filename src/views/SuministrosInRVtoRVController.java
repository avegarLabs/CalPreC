...
public class SuministrosInRVtoRVController implements Initializable {

    private static SessionFactory sf;
    @FXML
    public Label temp;
    SuministrosInRVtoRVController suministrosInRVController;
    RenVarianteToRenglonObraController renVarianteToRenglonObraControllerSum;
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        suministrosInRVController = this;
        loadComponents();
        ...
        dataTable.setOnMouseClicked(event -> {
            ...
            sumRVView.getCodigo().setOnMouseClicked(event1 -> {
                if (!sumRVView.getCodigo().isSelected()) {
                    try {
                        FXMLLoader loader = new FXMLLoader(getClass().getResource("SuministroBajoEspecificacionComponentRV.fxml"));
                        Parent proot = loader.load();
                        BajoEspecificacionControllerComponetRV controller = loader.getController();
                        controller.pasarDatos(suministrosInRVController, sumRVView, obraId);
    ...
    public void datosRV(RenVarianteToRenglonObraController renVarianteToRenglonObraController, Integer id, double cant, int idOb) {
        idRV = id;
        cantid = cant;
        ...
        renVarianteToRenglonObraControllerSum = renVarianteToRenglonObraController;
    }
    ...
    public void saveSumUpdate(SumRVView sumRVView) {
        ...
        if (sumRVView != null) {
            ...
            sumRVViewUpdate = new SumRVView(sumRVView.getId(), check, sumRVView.getDescripcion(), sumRVView.getUm(), sumRVView.getCosto(), sumRVView.getCantidad(), sumRVView.getUsos(), sumRVView.getTipo());


            dataTable.getItems().add(sumRVViewUpdate);
        }
    ...
    public void saveUpdateResources(javafx.event.ActionEvent event) {
        ...
        renVarianteToRenglonObraControllerSum.createBajoespecificacionList(updateList);
    ...
    public void handleDesgloseAction(ActionEvent event) {
        ...
        if (sumRVView.getTipo().contentEquals("S")) {
            ...
            System.out.println("Cantidad: " + cantSemi);
        ...
        try {
            ...
            suministro.getSemielaboradosrecursosById().forEach(items -> {
                if (items.getRecursosByRecursosId().getTipo().contentEquals("1")) {
                    ...
                    checkCode.setSelected(true);
                    sumRVView = new SumRVView(items.getRecursosId(), checkCode, items.getRecursosByRecursosId().getDescripcion(), items.getRecursosByRecursosId().getUm(), Math.round(items.getRecursosByRecursosId().getPreciomn() * 100d) / 100d, items.getCantidad() * cantSemi / items.getUsos(), 1.0, items.getRecursosByRecursosId().getTipo());
    ...
    private ObservableList<SumRVView> getComponenteInJuegodeProducto(int id) {
        ...
        try {
            ...
            for (Juegorecursos items : suministro.getJuegorecursosById()) {
                if (items.getRecursosByRecursosId().getTipo().contentEquals("1")) {
                    checkCode = new JFXCheckBox();
                    checkCode.setText(items.getRecursosByRecursosId().getCodigo());
                    checkCode.setSelected(true);
    ...
    public void handleAgregarSuministroAction(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("SuministroBajoEspecificacionComponentRVAdd.fxml"));
            Parent proot = loader.load();
            BajoEspecificacionControllerComponetRVAdd controller = loader.getController();
            controller.pasarDatos(suministrosInRVController, obraId);
