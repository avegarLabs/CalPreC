...
public class BajoEspecificacionControllerComponetRV implements Initializable {

    private static SessionFactory sf;
    public Integer idObra;
    public Obra obraToCheck;
    SuministrosInRVtoRVController suministrosInRVtoRVControllerSum;
    BajoEspecificacionControllerComponetRV bajoEspecificacionControllerComponetRVt;
    ReportProjectStructureSingelton utSingelton = ReportProjectStructureSingelton.getInstance();
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        bajoEspecificacionControllerComponetRVt = this;
        df = new DecimalFormat("#.####");

    }

    public ObservableList<String> listToSugestionSuministros(String tag) {
        Session session = ConnectionModel.createAppConnection().openSession();
    ...
    public void pasarDatos(SuministrosInRVtoRVController suministrosInRVController, SumRVView sumRVView, int idOb) {

        suministrosInRVtoRVControllerSum = suministrosInRVController;
        ...
        obraToCheck = utSingelton.getObra(idOb);
        listToSugestionSuministros(obraToCheck.getSalarioBySalarioId().getTag().trim());
        TextFields.bindAutoCompletion(field_codigo, listSuministros);
        cantidad = sumRVView.getCantidad();
...
//        importe = Double.parseDouble(labelValue.getText()) * 100d / 100d;
            ...
            suministrosInRVtoRVControllerSum.saveSumUpdate(newSum);
    ...
    public void searchView(ActionEvent event) {
        ...
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("searhSuministroRV.fxml"));
            Parent proot = loader.load();
            SearchSuminitrosControllerRV controller = loader.getController();
            controller.pasController(bajoEspecificacionControllerComponetRVt);
    ...
    public void getSuminitroResult(SuministrosView suministrosView) {
        ...
        fieldCant.setText(String.valueOf(cantidad));
    ...
    public void handleBajoAddSum(ActionEvent event) {
        ...
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("nuevoSuminitroFromBajoEspRV.fxml"));
            Parent proot = loader.load();
            NuevoSuministroFromBajoRVController controller = loader.getController();
            controller.pasarParametros(bajoEspecificacionControllerComponetRVt, field_codigo.getText(), textDedscripcion.getText(), um, String.valueOf(precio));
