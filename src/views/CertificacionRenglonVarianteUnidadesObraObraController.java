...
public class CertificacionRenglonVarianteUnidadesObraObraController implements Initializable {
    ...
    public boolean AddRenglonBase(int idUO, String reglon) {
        falg = false;
        Session session = ConnectionModel.createAppConnection().openSession();
        ...
        try {
            ...
            if (unidadobra.getRenglonbase() == null) {
                unidadobra.setRenglonbase(reglon);
                session.update(unidadobra);
                falg = true;
            } else if (unidadobra.getRenglonbase() != reglon) {
                unidadobra.setRenglonbase(reglon);
                session.update(unidadobra);
                falg = true;
            }
            ...
            dataTable.setOnMouseClicked(event -> {
                if (event.getClickCount() == 2) {
                    ...
                    try {
                        ...
                        Parent proot = loader.load();
                        CertificarRenglonVarianteController controller = loader.getController();
                        controller.cargarUnidadeObra(unidadObraView.getId(), empresaconstructoraModel.getId(), zonasModel.getId(), objetosModel.getId(), nivelModel.getId(), especialidadesModel.getId(), subespecialidadesModel.getId(), nivel.getObraId());


                        Stage stage = new Stage();
                        stage.setScene(new Scene(proot));
                        paneCalc.getChildren().setAll(proot);
 ...
 */
    ...
    public void handleDeleteRV(ActionEvent event) {
        ...
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setHeaderText("Información");
        alert.setContentText("Renglón Variante eliminado de la unidad de Obra!");
        alert.showAndWait();
    ...
    public void handleDeleteSumistro(ActionEvent event) {
        ...
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setHeaderText("Información");
        alert.setContentText("Suministro eliminado de la unidad de Obra!");
        alert.showAndWait();
