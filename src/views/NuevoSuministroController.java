...
public class NuevoSuministroController implements Initializable {
    ...
    public void addSuministroAction(javafx.event.ActionEvent event) {
        ...
        sumuni.setDescripcion(text_descripcion.getText().trim());
        ...
        if (getRecursoId(sumuni.getCodigo()) == 0) {
            ...
            list = getRecursosList(idOb);
            list.size();

        } else {
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setHeaderText("Información");
            alert.setContentText("Suministro existente en la obra");
            alert.showAndWait();
        }
    ...
    public void pasarParametros(Integer idObra) {
        if (idObra != null) {
            idOb = idObra;
            List<Recursos> list = new ArrayList<>();
            list = getRecursosList(idOb);
